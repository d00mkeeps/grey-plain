...
const NUM_LAYERS = 32
const LAYER_NETWORK = [
  'motor','motor','motor','visual','visual','visual',
  'language','language','language','language','language','language',
  'language','language','attention','attention','attention','attention',
  'dmn','dmn','dmn','dmn','attention','attention',
  'attention','attention','language','language','dmn','dmn',
  'dmn','dmn',
]
...
export default function Workspace3D({ currentTurn, tokenIndex, onResetCamera }) {
  ...
  useEffect(() => {
    ...
    sceneRef.current = {
      ...
      brainMesh: null, vertexRegions: null, regionMap: null,
      llmLayers: []
    }
    ...
    Promise.all([
      ...
      Object.keys(regionVerts).forEach(k => {
        ...
        vlist.sort((a, b) => {
          ...
          return da - db   // nearest centroid first so fill grows outward
        })
    ...
    for (let i = 0; i < NUM_LAYERS; i++) {
      const geo      = new THREE.CylinderGeometry(DISC_R, DISC_R, DISC_H, 32, 1, false, 0, Math.PI * 2)
      const ghostMat = new THREE.MeshPhongMaterial({
        color: LLM_GHOST_COLOR, transparent: true, opacity: 0.2,
        shininess: 30, wireframe: true
      })
      const ghostMesh = new THREE.Mesh(geo, ghostMat)
      ghostMesh.position.y = i * (DISC_H + GAP)

      const fillGeo  = new THREE.CylinderGeometry(DISC_R * 0.98, DISC_R * 0.98, DISC_H * 0.98, 32)
      const fillMat  = new THREE.MeshPhongMaterial({
        color: 0xffffff, transparent: true, opacity: 0, shininess: 50
      })
      const fillMesh = new THREE.Mesh(fillGeo, fillMat)
      fillMesh.position.y = i * (DISC_H + GAP)
      fillMesh.scale.set(0.001, 1, 0.001)
      fillMesh.userData = { targetScale: 0.001, targetOpacity: 0, targetColor: new THREE.Color(0xffffff) }
    ...
    const animate = () => {
      ...
      if (sceneRef.current.humanSprite && sceneRef.current.targetHumanColor)
        sceneRef.current.humanSprite.material.color.lerp(sceneRef.current.targetHumanColor, 0.1)
      if (sceneRef.current.llmSprite && sceneRef.current.targetLlmColor)
        sceneRef.current.llmSprite.material.color.lerp(sceneRef.current.targetLlmColor, 0.1)

      if (sceneRef.current.llmLayers) {
        sceneRef.current.llmLayers.forEach(({ fill }) => {
          if (fill.userData.targetScale !== undefined) {
            const s = fill.userData.targetScale < 0.01 ? 0.30 : 0.12
            fill.scale.x += (fill.userData.targetScale - fill.scale.x) * s
            fill.scale.z += (fill.userData.targetScale - fill.scale.z) * s
            fill.material.opacity += (fill.userData.targetOpacity - fill.material.opacity) * s
    ...
    sceneRef.current.resetCamera = () => { camera.position.set(0, 0, 320); controls.target.set(0, 0, 0) }
  ...
  useEffect(() => {
    ...
    sceneRef.current.targetHumanColor = humanData ? new THREE.Color('#7ac0f0') : new THREE.Color('#3a5a7a')
    sceneRef.current.targetLlmColor   = llmData   ? new THREE.Color('#7ac0f0') : new THREE.Color('#3a5a7a')
    ...
    if (brainMesh && vertexRegions && regionMap && renderer) {
      ...
      if (humanData && sortedLists) {
        Object.entries(activationByName).forEach(([name, activation]) => {
          ...
          // find the regionMap entry for this name
          const regionIdx = Object.keys(regionMap).find(k => regionMap[k]?.name === name)
          if (!regionIdx || !sortedLists[regionIdx]) return

          const vlist = sortedLists[regionIdx]
          const t     = Math.min(1, activation / 0.7)          // 0 → 1
          const count = Math.max(1, Math.round(t * vlist.length)) // vertices to light
    ...
    if (llmLayers?.length > 0) {
      const layerData = llmData?.layerActivations?.[tokenIndex] ?? Array(NUM_LAYERS).fill(0)
